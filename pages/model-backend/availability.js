...
console.log('Timestamp in EST timezone:', timestamp);
  ...
  const handleAvaiability = async (e) => {
    ...
    try {
      ...
      const response = await axios.post('https://api.tsm.cc/v1/update-model-time.php', formData);
  ...
  const saveUnavailablity = async (e) => {
    ...
    try {
      ...
      const response = await axios.post('https://api.tsm.cc/v1/update-uncertain-unavailability.php', unavailSubmitData);
  ...
  const saveWeeklyData = async (e) => {
    ...
    try {
      ...
      const response = await axios.post('https://api.tsm.cc/v1/update-weekly-avail-time.php', formDataWeekSubmit);
      console.log('rest', response.data);
      if(response.data == '1') {
  ...
  useEffect(() => {
    ...
    setId(modelid);
    let url = "https://api.tsm.cc/v1/single-model.php?id=" + localStorage.getItem("token");
